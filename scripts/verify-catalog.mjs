...
const evidenceSource=fs.readFileSync(new URL("../lib/evidence.ts",import.meta.url),"utf8");
const corpus=["catalog.ts","catalog-types.ts","evidence.ts","entities.ts","legacy-agents.ts"].map((name)=>fs.readFileSync(new URL(`../lib/${name}`,import.meta.url),"utf8")).join("\n");
const hashes=[...evidenceSource.matchAll(/contentHash:\s*"sha256:([0-9a-f]+)"/g)].map((m)=>m[1]);
if(!hashes.length) throw new Error("No evidence hashes found");
const bad=hashes.filter((h)=>h.length!==64);
